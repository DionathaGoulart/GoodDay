          ...
          children: [
            if (!isMinimalist)
              ...
              Padding(
                padding: const EdgeInsets.symmetric(vertical: 8.0),
                child: Text(
                  (category.emoji != null && category.emoji!.isNotEmpty ? '${category.emoji} ' : '') + category.name, 
                  style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
                ),
              ),
            
            if (isMinimalist)
              // Minimalist: Checkbox List
              Column(
                children: items.map((item) {
                  final isSelected = selectedItemIds.contains(item.id);
                  return CheckboxListTile(
                    title: Text(
                      (item.emoji != null && item.emoji!.isNotEmpty ? '${item.emoji} ' : '') + item.name,
                      style: TextStyle(
